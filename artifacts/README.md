...
Per-query performance table (equivalence and slot reduction per query)

Condition-wise performance table (aggregated by topk and min_similarity)
...
1️⃣ Run the Evaluation (Raw Data Collection)
...
2️⃣ Generate the Per-Query Summary Table
...
3️⃣ Generate the Condition-wise Aggregated Table
...
5️⃣ Mapping to Dissertation Tables
Dissertation Table	Source Script(s)	Description
Table A – Per-Query Performance	bq_cost_eval.py → summarize_bq_eval.py	Compares equivalence and slot time improvements per query (e.g., Q3, Q6, Q14, Q9, Q21).
Table B – Parameter Sensitivity	bq_cost_eval.py → aggregate_results.py	Aggregates performance across topk × min_similarity conditions.
